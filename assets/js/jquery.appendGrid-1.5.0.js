...
(function ($) {
    ...
    var _defaultColumnOptions = {
        ...
        onChange: null,
        // The 'OnKeyup' event callback of control. Added by ferrika
        onKeyup: null
    };
    var _systemMessages = {
        ...
        for (var z = 0; z < numOfRow; z++) {
            ...
            for (var y = 0; y < settings.columns.length; y++) {
                ...
                if (settings.columns[y].type != 'custom') {
                    ...
                    if ($.isFunction(settings.columns[y].onKeyup)) {
                        $(ctrl).keyup({ caller: tbWhole, callback: settings.columns[y].onKeyup, uniqueIndex: uniqueIndex }, function (evt) {
                            evt.data.callback(evt, $(evt.data.caller).appendGrid('getRowIndex', evt.data.uniqueIndex));
                        });
                    }
                }
